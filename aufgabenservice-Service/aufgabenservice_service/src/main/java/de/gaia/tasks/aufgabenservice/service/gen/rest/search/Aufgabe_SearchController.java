...
@ExposesResourceFor(Aufgabe_.class)
@RequestMapping("/aufgabes/search")
public class Aufgabe_SearchController {
		
    @Autowired
    QueryService service;

    @Autowired
    Aufgabe_Repository repository;
    ...
    public ResponseEntity<?> findFullTextFuzzy(PersistentEntityResourceAssembler assembler, @Param("q") String q) {
        ...
        String[] annotatedFields = Stream
                .of(Aufgabe_.class.getDeclaredFields())
                .filter(field -> field.isAnnotationPresent(org.hibernate.search.annotations.Field.class))
                .map(Field::getName)
                .toArray(size -> new String[size]);

        Stream<Aufgabe_> aufgabeStream;
        try {
            aufgabeStream = service.query(q, Aufgabe_.class, annotatedFields).stream();
        } catch (EmptyQueryException e) {
            aufgabeStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final List<PersistentEntityResource> collect = aufgabeStream.map(assembler::toResource).collect(Collectors.toList());
