...
import de.gaia.tasks.aufgabenservice.api.businessactions.BusinessActionsRestClientImpl;
 ...
 * @version 1.0
 */
@SpringComponent("AufgabenService_BusinessActionsService")
@UIScope
public class BusinessActionsServiceImpl implements BusinessActionsService {
	 ...
	 */
	@Autowired
	public BusinessActionsServiceImpl(SecurityService securityService, @Value("${AUFGABENSERVICE.microservice.basePath}") String basePath) {
	...
	public void mitarbeiterInformieren(){
		Future<?> result = executor.submit(() -> client.mitarbeiterInformieren());
		try {
			...
			LOG.error(e.getMessage());
			showErrorNotification(I18nPaths.NotificationType.error, SimpleAction.none, TIMEOUT_I18N);
		} catch (Exception e) {
			LOG.error(e.getMessage());
			showErrorNotification(I18nPaths.NotificationType.error, SimpleAction.none);
		} finally {
